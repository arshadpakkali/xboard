...
class UserService {
    ...
    async getUserByToken(token) {
        ...
        console.log(id);
        let user = await model.findOne({emailAddress:id.emailAddress.trim().toUpperCase()}, {password: 0});
        console.log(user);
        return user;
    }
    async set_products_for_user(request){
        ...
        if(!user){
            ...
            if(user_product !== -1) {
                ...
                if( new Date(user.products[user_product].expiry).valueOf() < Date.now()) {
                    user.products[user_product].expiry = Date.now() + request.expiry*24*60*60*1000;
                } 
                // if expiry date greater than equal to Date now
                else  {
                    user.products[user_product].expiry = new Date(user.products[user_product].expiry).valueOf() + request.expiry*24*60*60*1000;
                }
                
            } else {
                let product = {product_id: request.product_id, expiry: Date.now() + request.expiry*24*60*60*1000};
