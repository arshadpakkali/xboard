  ...
  try {
    if (!req.query.token) {
      res.render("invalid_token", {
        data: "Looks like this request is not authenticated",
      });
    } else {
      let verification = await jwt.verify(
        req.query.token,
        "arstrdjjthdtdhgdtychgkhkdty"
      );
      if (!verification) {
        res.render("invalid_token", {
          data: "Looks like this request is not correct",
        });
      } else {
    ...
    console.log(err);
    res.render("invalid_token", {
      data: "Something is wrong, Please contact administrator",
    });
    // next(err);
  }
});
...
app.use(express.static(path.join(__dirname, "../dist/super-dashboard")));
